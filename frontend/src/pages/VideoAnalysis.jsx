...
import { Upload, Video, Brain, AlertTriangle, CheckCircle, RefreshCw } from 'lucide-react'
import api from '../api'
    ...
    <div className="video-analysis-page fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="subtitle">Upload a video to get an AI-powered emotional & health assessment</p>
        </div>
        ...
        >
          ...
          {!file ? (
            <div className="upload-prompt">
              ...
              <p className="upload-hint">The AI will analyze facial expressions frame by frame to assess emotional and physical wellbeing</p>
            </div>
          ) : (
        ...
        <div className="results-grid fade-in">
          ...
          <div className="card">
            ...
            <div className="emotion-bars">
              {Object.entries(result.emotion_summary || {})
                .sort((a, b) => b[1] - a[1])
                .map(([emotion, pct]) => (
                  <div key={emotion} className="emotion-bar-row">
                    ...
                    <div className="emo-bar-track">
                      <div className="emo-bar-fill"
                        style={{ width: `${pct}%`, background: EMOTION_COLOR[emotion] || '#94a3b8' }} />
                    </div>
          ...
          {result.health_indicators?.length > 0 && (
            <div className="card">
              ...
              <div className="concern-list">
                {result.health_indicators.map((c, i) => (
                  <div key={i} className="concern-item">⚠️ {c}</div>
                ))}
              </div>
            </div>
          )}
          ...
          {result.recommendations?.length > 0 && (
            <div className="card">
              <h3><CheckCircle size={16} color="var(--accent-green)"/> Recommendations for Caregiver</h3>
              <div className="rec-list">
                {result.recommendations.map((r, i) => (
                  <div key={i} className="rec-item-v">✅ {r}</div>
                ))}
              </div>
            </div>
          )}
